...
def get_aperture_distribution(session: Session, lens: str | None = None) -> list[dict]:
    query = (
...
def get_gear_usage(session: Session) -> list[dict]:
    ...
    for model, make, count, first, last in lenses:
        gear.append({"type": "lens", "make": make, "model": model, "count": count, "first_used": first, "last_used": last})
    for model, make, count, first, last in bodies:
        gear.append({"type": "body", "make": make, "model": model, "count": count, "first_used": first, "last_used": last})
...
def get_map_data(session: Session) -> list[dict]:
    ...
    return [{"lat": lat, "lon": lon, "lens": lens, "date": dt} for lat, lon, lens, dt in results]
